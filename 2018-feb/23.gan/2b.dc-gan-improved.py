from keras.layers import Input, Dense, Activation, LeakyReLU, Reshape, Conv2D, Conv2DTranspose, Activation, BatchNormalization, Flatten
...
def build_gan(latent_sample_size, d_optimizer, g_optimizer):
    ...
    descriminator.compile(optimizer=d_optimizer, loss='binary_crossentropy', metrics=['accuracy'])
    gan.compile(optimizer=g_optimizer, loss='binary_crossentropy', metrics=['accuracy'])
...
def make_labels(size):
    return np.random.uniform(0.7, 1.2, size), np.random.uniform(0.0, 0.3, size)
...
def train_gan(generator, descriminator, gan, X_train_real, batch_size, epochs, latent_sample_size):
    ...
    for e in range(epochs):
        ...
        for i in range(nbatches):
            ...
            d_loss = metrics[0]
            d_acc = metrics[1]
            log = "%d/%d: [descriminator loss: %f, acc: %f]" % (e, i, d_loss, d_acc)
...
d_optimizer = Adam(lr=d_learning_rate)
g_optimizer = Adam(lr=g_learning_rate)
generator, descriminator, gan = build_gan(latent_sample_size, d_optimizer, g_optimizer)
