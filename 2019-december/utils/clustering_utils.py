...
def plot_model_2d_clustering(estimator, X, ax = None, xlim=None, ylim=None, title=None):
    name = str(estimator)
    if(name.startswith('Gaussian')):
        y = estimator.fit_predict(X)
    else:
    ...
    ax = plot_data_2d_classification(X, y, ax, xlim, ylim, title)
    if hasattr(estimator, 'cluster_centers_'):
        centers = estimator.cluster_centers_
        plot_data_2d(centers, ax, title = title, s=200)
